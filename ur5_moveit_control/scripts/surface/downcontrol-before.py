    ...
    if dh_uartqueue.length() >= 8:
        ...
        if listrecv_databuf[:3] == ['55', 'aa', '03']:
            if listrecv_databuf[3] == 'e1': # 链路检测
                ...
                robotcontrol.dh_robot_jointspeed(0.1) 
                robotcontrol.dh_robot_speedset(0.008)
                robotcontrol.dh_robot_topchangepos(3)
                ...
                if dh_uartqueue.length() >= 6:
                    ...
                    if listrecv_databuf[8] == '01':
                        ...
                        #robotcontrol.dh_robot_jointspeed(0.08) 
                        #robotcontrol.dh_robot_speedset(0.025)
                        #robotcontrol.dh_robot_posmove('pos_y', +0.03) #0.12 1cm                        
                        #print("机械臂向左移动")  
                    elif listrecv_databuf[8] == '05': # 向左移动
                        del listrecv_databuf[:] #清空列表
                        dh_uart_send('MutexCapTrack')
                        #print("Exe MutexCap Track")
                        #robotcontrol.dh_robot_jointspeed(0.08) 
                ...
                if dh_uartqueue.length() >= 6:
                    ...
                    if listrecv_databuf[8] == '01':
                        ...
                        del listrecv_databuf[:] #清空列表
                        print("抓取开始状态量反馈")
                        robotcontrol.dh_robot_jointspeed(0.02) 
                        robotcontrol.dh_robot_speedset(0.009)
                        robotcontrol.dh_robot_poschange('pos_z', +0.12) #0.12 1cm
...
def dh_uart_listdata_analysis_one():
    listrecv_databuf = [] 
    """列表数据解析"""
    if dh_uartqueue.length() >= 8:
        count_num = 4
            ...
            if listrecv_databuf[3] == 'e1': # 链路检测
                ...
                if dh_uartqueue.length() >= 6:
                    ...
                    if listrecv_databuf[8] == '01':
                        ...
                        del listrecv_databuf[:] #清空列表
                        print("抓取开始状态量反馈")
                        robotcontrol.dh_robot_speedset(0.04)
                        robotcontrol.dh_robot_poschange('pos_z', 0.12) #0.01 1cm
                        time.sleep(5)
                        robotcontrol.dh_robot_jointspeed(0.2) 
                        robotcontrol.dh_robot_speedset(0.008)
                        robotcontrol.dh_robot_topchangepos(1)
                        time.sleep(3)
                        robotcontrol.dh_robot_jointspeed(0.4) 
                        #robotcontrol.dh_robot_speedset(0.008)
                        robotcontrol.dh_robot_topchangepos(2)
                        robotcontrol.dh_robot_jointspeed(0.2) 
                        robotcontrol.dh_robot_speedset(0.008)
                        robotcontrol.dh_robot_topchangepos(3)
                        time.sleep(1)
                        dh_uart_send('ProjectEnd')

                        #robotcontrol.dh_robot_poschange('pos_x', +0.05) #0.01 1cm
                        #robotcontrol.dh_robot_poschange('pos_z', -0.05) #0.01 1cm
                        #robotcontrol.dh_robot_poschange('pos_x', 0.05) #0.01 1cm
                        #robotcontrol.dh_robot_speedset(0.002)
                        #robotcontrol.dh_robot_circular()
