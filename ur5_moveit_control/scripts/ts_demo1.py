...
while True:
    ...
    for object in resp.obj:
        if object.label == "up_bottle_spring":
            if object.conf < threshold:                                 # object.conf 表示物品识别的置信度，大于 0.7 表示识别结果可信，可按照实际需求及表现进行修改
                ...
                while True:
                    ...
                    point = ts.get_point_client(object.colu, object.row)      # (160  275)桌面裁剪左上角坐标点
        ...
        if object.label == "bottle_spring": 
            if object.conf < threshold:                                 # object.conf 表示物品识别的置信度，大于 0.7 表示识别结果可信，可按照实际需求及表现进行修改
                ...
                while True:
                    ...
                    point = ts.get_point_client(object.colu, object.row)
