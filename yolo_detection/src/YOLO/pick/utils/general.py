...
from YOLO.pick.utils import polyiou
from YOLO.pick.utils.google_utils import gsutil_getsize
from YOLO.pick.utils.torch_utils import init_torch_seeds, is_parallel
...
def plot_one_rotated_box(rbox, img, color=None, label=None, line_thickness=None, pi_format=True):
    ...
    if label:
        ...
        cv2.rectangle(img, c1, c2, color, -1, cv2.LINE_AA)  # filled
        cv2.putText(img, str(label+str((c1[0], c1[1] - 2))+str(int(180-rbox[-1]))), (c1[0], c1[1] - 2), 0, tl / 4, [0, 0, 0], thickness=tf, lineType=cv2.LINE_AA)
        # cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl / 4,
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=4):
    ...
    for i, img in enumerate(images):
        ...
        max_w_h=max(w,h)
        block_x = int(max_w_h * (i // ns))
        block_y = int(max_w_h * (i % ns))
